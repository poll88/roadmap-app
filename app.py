...
from lib.ids import ensure_stable_ids
...
# Handle delete/reset flags BEFORE widgets render (prevents widget key errors)
...
# Act on sidebar results (rerun only for explicit user actions)
if actions.get("add"):
    st.rerun()
if actions.get("edit"):
    st.rerun()
...
if not st.session_state["items"]:
    ...
    # Visual timeline (selection is driven by the picker for reliability)
